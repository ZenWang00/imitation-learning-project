...
class ContinuousCSILAgent(ImitationAgent):
    """Coherent Soft Imitation Learning with continuous actions (Pendulum-v1).

    Implements Watson et al., NeurIPS 2023.

    Training has two phases:
      1. BC pretraining: fit bc_actor to expert demonstrations via NLL.
      2. RL fine-tuning: SAC with the coherent reward
         r(s,a) = alpha * (log pi_BC(a|s) - log N(a;0,I)).

    bc_actor is frozen after pretraining. rl_actor is initialized from bc_actor
    weights and then fine-tuned with SAC.
    """
    ...
    def pretrain(
        ...
        The rl_actor is then warm-started from bc_actor weights.
        """
        self.bc_actor.train()
        ...
        # Warm-start the RL actor from BC weights so fine-tuning starts coherently.
        self.rl_actor.load_state_dict(self.bc_actor.state_dict())
        self.rl_actor.requires_grad_(True)
        self.rl_actor.train()
        self.actor_optimizer = torch.optim.Adam(
            self.rl_actor.parameters(), lr=self.actor_lr
        )
