...
class SOARCSILAgent(ImitationAgent):
    """SOAR-enhanced Coherent Soft Imitation Learning with discrete actions (CartPole-v1).

    Extends CSILAgent with a state occupancy discriminator (SOAR).
    Total reward: r(s,a) = r_coherent(s,a) + soar_weight * log_odds_D(s)
    where r_coherent is the CSIL coherent reward from the frozen BC policy.
    """
    ...
    def update_with_replay(
        ...
        # 1. Update SOAR state discriminator
        ...
        with torch.no_grad():
            coherent_rewards = coherent_reward_discrete(
                replay_batch.observations,
        ...
        # 3. Update Q-network with combined reward
